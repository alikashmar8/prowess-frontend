  ...
  async searchClicked(data?: {
    ...
    try {
      ...
      this.filters = {
        ...
        isPaid: ![undefined, '', null].includes(data.isPaid)
          ? Boolean(data.isPaid)
          : null,
        ...
        take: data.take || this.take,
        skip: data.skip || this.skip,
      };
      const res = await this.invoicesService.listInvoices(this.filters);
      this.invoices = res.data;
      this.sum = res.sum;
      this.totalRecords = res.count;
      this.totalPages = res.count > 0 ? Math.ceil(res.count / this.take) : 1;
